...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(train_dataloader, start=epoch_iter):
        ...
        print("yyyyyyyyyyyyyyyyyyyyy",data["seg_map"].shape,data["dense_map"].shape,data["target"].shape,data["seg_mask"].shape)
        ############## Forward Pass ######################
        # Reference Purpose
        #input_dict = {'seg_map': A_tensor, 'dense_map': dense_img, 'target': B_tensor, 'seg_map_path': A_path,'target_path': A_path, 'densepose_path': dense_path }
        #print( data['seg_mask'].shape)
        #print("data",data.keys())
        #print("print model===========================================",len(model(data["seg_map"],data["dense_map"],data["target"],data["seg_mask"])))
